...
import { CheckIcon, ChevronDownIcon, Columns3Icon, PlusIcon, XIcon } from "lucide-react"
...
import { operatorWords } from "@/flows/sprint-4/idea-1/grammar"

/**
 * Everything the sentence can do to the query. The same edits the columns and
 * the logic gate make, by the same functions, so no view can do something the
 * others cannot show.
 */
export interface QueryHandlers {
  ...
  /** Open an attribute, and optionally one of its values, in the Miller columns. */
  onShowInColumns: (attribute: string, value?: string) => void
  /** Per-value counts for one attribute, with its own condition lifted. */
  countsFor: (attribute: string) => Record<string, number>
}

/**
 * The value list behind a pill: every value of the attribute with what picking
 * it would leave, and the way back to the same attribute in the columns.
 */
function ValueMenu({
  condition,
  handlers,
  ...
  const counts = handlers.countsFor(condition.attribute)
  const options = valueOptions(condition.attribute, condition.values)
  ...
  return (
    <Command>
      ...
      <CommandInput placeholder={`Search ${condition.attribute.toLowerCase()}…`} />
      <CommandList className="max-h-[260px]">
          ...
          {options.map((value) => {
            ...
            return (
              <CommandItem
                key={value}
                value={value}
                onSelect={() => handlers.onToggleValue(condition.attribute, value)}
                className="gap-2"
              >
                <span
                  className={cn(
                ...
                <span
                  className={cn(
                    ...
                    (counts[value] ?? 0) === 0 ? "text-muted-foreground/60" : "text-muted-foreground",
                  )}
                >
                  {(counts[value] ?? 0).toLocaleString("en-GB")}
                </span>
              </CommandItem>
            )
          })}
        ...
        <CommandGroup>
          <CommandItem
            onSelect={() => {
              onClose()
              handlers.onShowInColumns(condition.attribute)
            }}
          ...
          <CommandItem
            onSelect={() => {
              onClose()
              handlers.onRemoveCondition(condition.attribute)
            }}
            className="text-muted-foreground gap-2"
          >
    ...
    <span className="group/pill relative mr-1 -ml-0.5 inline-flex align-baseline">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent align="start" className="w-[320px] p-0 text-sm">
          <ValueMenu condition={condition} handlers={handlers} onClose={() => setOpen(false)} />
        </PopoverContent>
      </Popover>
...
/**
 * `+ condition`. A condition with no values says nothing, so picking an
 * attribute here opens it in the columns, where its values are counted.
 */
function AddCondition({ conditions, handlers }: { conditions: Condition[]; handlers: QueryHandlers }) {
  ...
  const available = drugAttributeOrder.filter(
    (attribute) => attributeDefs[attribute] && !used.has(attribute),
  )
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="max-h-80 w-[260px] overflow-y-auto">
        ...
        {available.map((attribute) => (
          <DropdownMenuItem key={attribute} onSelect={() => handlers.onShowInColumns(attribute)}>
            {attribute}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
