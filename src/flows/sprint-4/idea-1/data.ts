/**
 * Static content for Sprint 4 / Idea 1 — the hybrid screener.
 *
 * Copied from Sprint 3 Idea 2 rather than imported, because the directions stay
 * independent: the same fixed sample of 1,440 drug rows, built once at module
 * load from weighted tables and a seeded generator, and the same taxonomy the
 * Miller columns browse. Every number on screen — the headline, the per-value
 * counts in the columns and in a pill's menu, the running count on each node of
 * the logic gate — is counted off that one array against the query as it
 * stands, so the four views of the query cannot disagree.
 *
 * What changed on the way across is the query model. Idea 2 joined its
 * attributes with an implicit AND; the logic gate draws every join as a node
 * you can flip, so each condition here carries its own link to the one before.
 */
 ...
 * view draws the same list: a clause of the sentence, a group card in the logic
 * gate, the ticks in one attribute's column.
 */
 ...
 * same rows its column would show.
 */
export function attributeSplit(conditions: Condition[]) {
  const counts: Record<string, number> = {}
...
/** The top-level value a child label sits under, so the columns can open on it. */
export function parentOf(label: string) {
  return Object.entries(childrenByValue).find(([, items]) =>
