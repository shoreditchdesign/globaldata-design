...
import { PaneHeading } from "@/flows/sprint-2/idea-1/components/FilterModal"
import { filterAreas } from "@/flows/sprint-2/idea-1/data"
...
export function CascadePanel({
  ...
  return (
    <div
      className={cn(
        "bg-background absolute w-[280px] overflow-hidden rounded-lg border shadow-lg",
        className,
      )}
    >
      <div className="bg-muted/60 text-muted-foreground flex items-center gap-1.5 border-b px-3 py-2.5 text-[10px] font-medium tracking-[0.08em] uppercase">
        {breadcrumb ? (
          <>
      ...
      <div className="text-muted-foreground flex items-center gap-2 border-b px-3 py-2.5 text-sm">
        <SearchIcon className="size-3.5" />
        {searchPlaceholder}
      </div>

      <div className="max-h-[420px] overflow-y-auto py-1">{children}</div>

      <div className="text-muted-foreground flex items-stretch border-t text-[10px] font-medium tracking-[0.08em] uppercase">
